...
if (isset($_POST['create'])) {
    extract($_POST);
    if (!empty($category_id) && !empty($sub_category_id) && !empty($name) && !empty($short_details) && !empty($new_price)) {
 ...
 $sqlpr=$obj->FlyQuery("SELECT MAX(priority) as pr FROM `".$table."`");
        ...
        $insert = array('category_id' => $category_id, 'sub_category_id' => $sub_category_id, 'name' => $name, 'photo' => $photo, 'short_details' => $short_details, 'long_details' => $long_details, 'new_price' => $new_price, 'old_price' => $old_price, 'quantity' => $quantity,'priority'=>$pr, 'date' => date('Y-m-d'), 'status' => 1);
    ...
    if (!empty($category_id) && !empty($sub_category_id) && !empty($name) && !empty($short_details) && !empty($new_price)) {
        ...
        if (!empty($_FILES['photo']['name'])) {
            ...
            $photo = $ex_photo;
        }$upd2 = array('category_id' => $category_id, 'sub_category_id' => $sub_category_id, 'name' => $name, 'photo' => $photo, 'short_details' => $short_details, 'long_details' => $long_details, 'new_price' => $new_price, 'old_price' => $old_price, 'quantity' => $quantity, 'date' => date('Y-m-d'), 'status' => 1);
